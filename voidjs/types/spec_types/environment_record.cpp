...
bool DeclarativeEnvironmentRecord::HasBinding(VM* vm, String* N) const {
...
void DeclarativeEnvironmentRecord::CreateMutableBinding(VM* vm, String* N, bool D) {
  // 1. Let envRec be the declarative environment record for which the method was invoked.
...
void DeclarativeEnvironmentRecord::SetMutableBinding(VM* vm, String* N, JSValue V, bool S) {
  // 1. Let envRec be the declarative environment record for which the method was invoked.
...
JSValue DeclarativeEnvironmentRecord::GetBindingValue(VM* vm, String* N, bool S) const {
  // 1. Let envRec be the declarative environment record for which the method was invoked.
...
bool DeclarativeEnvironmentRecord::DeleteBinding(VM* vm, String* N) {
  // 1. Let envRec be the declarative environment record for which the method was invoked.
...
JSValue DeclarativeEnvironmentRecord::ImplicitThisValue(VM* vm) const {
  // 1. Return undefined.
  return JSValue::Undefined();
}

void DeclarativeEnvironmentRecord::CreateInmmutableBinding(VM* vm, String* N) {
  // 1. Let envRec be the declarative environment record for which the method was invoked.
  // 2. Assert: envRec does not already have a binding for N.
...
void DeclarativeEnvironmentRecord::InitializeImmutableBinding(VM* vm, String* N, JSValue V) {
  // 1. Let envRec be the declarative environment record for which the method was invoked.
...
bool ObjectEnvironmentRecord::HasBinding(VM* vm, String* N) const {
  ...
  return Object::HasProperty(vm, object_, N);
}
...
void ObjectEnvironmentRecord::CreateMutableBinding(VM* vm, String* N, bool D) {
  ...
  Object::DefineOwnProperty(vm, object_, N,
                             PropertyDescriptor(JSValue::Undefined(), true, true, config_value),
                             true);
}

void ObjectEnvironmentRecord::SetMutableBinding(VM* vm, String* N, JSValue V, bool S) {
  // 1. Let envRec be the object environment record for which the method was invoked.
  // 2. Let bindings be the binding object for envRec.
  // 3. Call the [[Put]] internal method of bindings with arguments N, V, and S.
  Object::Put(vm, object_, N, V, S);
}
...
JSValue ObjectEnvironmentRecord::GetBindingValue(VM* vm, String* N, bool S) const {
  ...
  auto value = Object::HasProperty(vm, object_, N);
  ...
  return Object::Get(vm, object_, N);
}
...
bool ObjectEnvironmentRecord::DeleteBinding(VM* vm, String *N) {
  ...
  return Object::Delete(vm, object_, N, false);
}
